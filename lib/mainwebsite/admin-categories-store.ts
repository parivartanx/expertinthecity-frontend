...
export const useAdminCategoriesStore = create<AdminCategoriesState>()(
  persist(
    (set, get) => ({
      ...
      deleteSubcategory: async (id: string) => {
        try {
          ...
          if (error.response?.status === 401) {
            ...
            errorMessage = error.response.data.error || "Cannot delete subcategory";
          } else if (error.response?.status === 404) {
            errorMessage = "Subcategory not found";
          } else if (error.response?.data?.error) {
            errorMessage = error.response.data.error;
      ...
      setCurrentSubcategory: (subcategory: Subcategory | null) => set({ currentSubcategory: subcategory }),
      ...
      refreshData: async () => {
        const { fetchAllCategories, fetchAllSubcategories } = get();
        try {
          await Promise.all([
            fetchAllCategories(),
