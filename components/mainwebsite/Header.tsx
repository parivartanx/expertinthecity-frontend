...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`w-full px-4 py-2 flex items-center justify-between fixed top-0 z-[999] h-[60px] transition-all duration-300 ${pathname === "/" || pathname === "/"
          ? isScrolled
        ...
        <Link href="/" className="flex items-center gap-2 group">
          <div className="relative flex items-center gap-2">
            ...
            <div className="relative">
              <HiOfficeBuilding
                className={`text-3xl ${(pathname === "/" || pathname === "/") && !isScrolled
                  ? "text-white"
                  : "text-green-600"
            ...
            <div className="flex flex-col leading-tight">
              <div className="flex items-center gap-1">
                {/* Main "Expert" text */}
                <span className="text-2xl font-extrabold text-green-600 transition-colors duration-300">
                  Expert
                </span>

                {/* "in" text */}
                <span
                  className={`text-xs font-medium ${(pathname === "/" || pathname === "/") && !isScrolled
                    ? "text-white/90"
                    : "text-green-600"
                    } -mt-1`}
                >
              ...
              {/* "THE CITY" text */}
              <span
                className={`text-sm font-bold tracking-wider ${(pathname === "/" || pathname === "/") && !isScrolled
                  ? "text-white"
                  : "text-green-600"
                  } -mt-1`}
              >
        ...
        <div className="hidden md:flex items-center ml-8">
          <div
            className={`relative flex items-center ${(pathname === "/" || pathname === "/") && !isScrolled
              ...
              } border rounded-full px-4 py-1.5 w-[280px]`}
          >
            <CiSearch
              className={`text-xl ${(pathname === "/" || pathname === "/") && !isScrolled
                ? "text-white/80"
                : "text-neutral-500"
                }`}
            />
            <input
              ...
              className={`w-full bg-transparent border-none outline-none px-2 text-sm ${(pathname === "/" || pathname === "/") && !isScrolled
            ...
            {searchFocused && searchQuery.trim() && (
              <>
                {isSearchLoading && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-lg border border-green-200/50 overflow-hidden z-50 p-4">
          ...
          <div className="flex gap-2 items-center">
            <div className="hidden lg:flex gap-2 ml-2">
              {isAuthenticated ? (
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="flex items-center gap-2 hover:opacity-80 transition-all">
                        ...
                        <span
                          className={`text-sm font-medium md:hidden ${pathname === "/" && !isScrolled
                            ? "text-white"
                            : "text-green-600"
                            }`}
                        >
                ...
                <Link
                  href="/login"
                  className={`px-4 py-2 rounded-lg font-medium transition-colors ${pathname === "/" && !isScrolled
      ...
      {mounted &&
        createPortal(
          ...
          >
            {/* Overlay */}
            <div
              className={`fixed inset-0 bg-black/40 backdrop-blur-sm transition-opacity duration-300 ${mobileOpen ? "opacity-100" : "opacity-0 pointer-events-none"
                }`}
              onClick={() => {
                setMobileOpen(false);
            ...
            >
              ...
              <div className="mb-4">
                ...
                {searchFocused && searchQuery.trim() && (
                  <>
                    {isSearchLoading && (
                      <div className="absolute left-4 right-4 mt-2 bg-white rounded-lg shadow-lg border border-green-200/50 overflow-hidden z-50 p-4">
                        <div className="flex items-center justify-center">
                          <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-green-600"></div>
                          <span className="ml-2 text-sm text-neutral-600">Searching...</span>
                        </div>
                  ...
                  !link.dropdown ? (
                    ...
                    <div key={link.name} className="flex flex-col gap-1">
                      ...
                      >
                        ...
                        <MdKeyboardArrowDown
                          className={`text-lg transition-transform duration-200 ${mobileDropdown ? "rotate-180" : ""
                            }`}
                        />
                      </button>
                      <div
                        className={`overflow-hidden transition-all duration-200 ${mobileDropdown ? "max-h-96" : "max-h-0"
                          }`}
                      >
              ...
              <div className="mt-4">
                {isAuthenticated ? (
                  <>
                    <div className="flex items-center gap-2 px-3 py-2 cursor-pointer hover:bg-green-50 rounded-lg transition-colors">
                      <Avatar className="h-8 w-8 border-2 border-green-600">
                        <AvatarImage src={user?.avatar} alt={user?.name} />
                        <AvatarFallback>
                          {getInitials(user?.name || "")}
                    ...
                    <button
                      ...
                      className="w-full border border-green-600 text-green-600 px-4 py-2 rounded-lg hover:bg-green-50 flex items-center justify-center gap-2 mt-3"
                    >
                  ...
                  <div className="flex flex-col gap-3">
                    <Link
                      href="/login"
                      className="w-full border border-green-600 text-green-600 px-4 py-2 rounded-lg hover:bg-green-50 flex items-center justify-center gap-2"
                      onClick={() => setMobileOpen(false)}
                    >
                      <IoIosLogOut className="text-lg" />
                      Login
